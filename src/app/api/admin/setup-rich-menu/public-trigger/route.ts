...
export async function GET() {
  try {
    ...
    const publicMenu: any = {
      ...
      selected: true,
      name: 'Public Restricted Menu V5 NEON',
    ...
    const driverMenu: any = {
      size: { width: 2500, height: 1686 },
      selected: true,
      name: 'Driver Hub Menu V5 NEON',
      chatBarText: 'เมนูคนขับรถ',
      areas: [
        { bounds: { x: 0, y: 0, width: 1250, height: 843 }, action: { type: 'message', text: 'ดูงาน' } },
        { bounds: { x: 1250, y: 0, width: 1250, height: 843 }, action: { type: 'message', text: 'ดูงาน' } },
        { bounds: { x: 0, y: 843, width: 1250, height: 843 }, action: { type: 'message', text: 'ดูโปรไฟล์' } },
        { bounds: { x: 1250, y: 843, width: 1250, height: 843 }, action: { type: 'message', text: 'กดส่งเอกสาร' } }
      ]
    };
    ...
    await Setting.findOneAndUpdate(
      ...
      { 
        ...
        standardReference: 'LINE CONFIG V5 NEON (AUTO)',
        isActive: true
      },
      { upsert: true }
    );
